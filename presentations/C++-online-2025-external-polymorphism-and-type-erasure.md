...
The code in `zoo::AnyContainer` refers to value management in identifiers and comments, the names of `ByValue` and `ByReference` are supposed to abbreviate `ManagementByValue` and `ManagementByReference`, and this makes clear that there could be other managers.  But these mechanisms belong to the 2nd generation of the framework.  These managers are reifications of a more abstract concept of "manager", which in the first generation I named as `Container`, this mistake, because the identification/articulation of Value Manager happened gradually while devising the 2nd. Generation of the framework.
