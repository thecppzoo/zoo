...
#define ZOO_TRACEABLE_EXP_IMPL(F, L, ...)                                      \
  std::cout << '"' << (__VA_ARGS__) << "\", \"" << F << ':' << L << "\", \""   \
            << ZOO_TO_STRING(__VA_ARGS__) << "\"" << std::endl;
#define ZOO_TRACEABLE_EXPRESSION(...)                                          \
  ZOO_TRACEABLE_EXP_IMPL(__FILE__, __LINE__, __VA_ARGS__)
...
template <typename S> constexpr auto parallelSuffix(S input) {
  auto log2Count = meta::logCeiling(S::NBits), power = 1;

  auto result = input, shiftMask = S{~S::MostSignificantBit};
...
template <int NB, typename B>
constexpr SWAR<NB, B> compress(SWAR<NB, B> input, SWAR<NB, B> compressionMask) {
...
template <int NB, typename B> constexpr SWAR<NB, B> parity(SWAR<NB, B> input) {
  using S = SWAR<NB, B>;
...
template <int NB, typename B> struct ArithmeticResultTriplet {
  SWAR<NB, B> result;
...
/// practical to use the carry bit for other than detection of unsigned
/// overflow.
///
/// The signed integer interpretation is two's complement, which
/// routinely overflows (when interpreted as unsigned).  Signed overflow may
/// only occur if the inputs have the same sign, it is detected when the sign of
/// the result is opposite that of the inputs.
///
...
template <int NB, typename B>
constexpr ArithmeticResultTriplet<NB, B> fullAddition(SWAR<NB, B> s1,
                                                      SWAR<NB, B> s2) {
  using S = SWAR<NB, B>;
  constexpr auto SignBit = S{S::MostSignificantBit},
                 LowerBits = SignBit - S{S::LeastSignificantBit};
  // prevent overflow by clearing the most significant bits
  auto s1prime = LowerBits & s1, s2prime = LowerBits & s2,
       resultPrime = s1prime + s2prime, s1Sign = SignBit & s1,
       s2Sign = SignBit & s2, signPrime = SignBit & resultPrime,
...
template <int NB, typename B>
constexpr SWAR<NB, B> saturatingUnsignedAddition(SWAR<NB, B> s1,
                                                 SWAR<NB, B> s2) {
...
template <int NB, typename B> constexpr auto negate(SWAR<NB, B> input) {
  using S = SWAR<NB, B>;
  constexpr auto Ones = S{S::LeastSignificantBit};
  return fullAddition(~input, Ones).result;
}

/// \brief Performs a generalized iterated application of an associative
/// operator to a base
///
/// In algebra, the repeated application of an operator to a "base" has
/// different names depending on the operator, for example "a + a + a + ... + a"
/// n-times would be called "repeated addition", if * is numeric multiplication,
/// "a * a * a * ... * a" n-times would be called "exponentiation of a to the n
/// power".
/// The general term in algebra is "iteration", hence the naming of this
/// function. Since * and "product" are frequently used in Algebra to denote the
/// application of a general operator, we keep the option to use the imprecise
/// language of "product, base and exponent".  "Iteration" has a very different
/// meaning in programming and especially different in C++. There may be
/// iteration over an operator that is not associative (such as quaternion
/// multiplication), this function leverages the associative property of the
/// operator to "halve" the count of iterations at each step. \note There is a
/// symmetrical operation to be implemented of associative iteration in the
/// "progressive" direction: instead of starting with the most significant bit
/// of the count, down to the lsb, and doing "op(result, base, count)"; going
/// from lsb to msb doing "op(result, square, exponent)" \tparam Operator a
/// callable with three arguments: the left and right arguments to the operation
/// and the count to be used, the "count" is an artifact of this generalization
/// \tparam IterationCount loosely models the "exponent" in "exponentiation",
/// however, it may not be a number, the iteration count is part of the
/// execution context to apply the operator \param forSquaring is an artifact of
/// this generalization \param log2Count is to potentially reduce the number of
/// iterations if the caller a-priori knows there are fewer iterations than what
/// the type of exponent would allow
template <typename Base, typename IterationCount, typename Operator,
...
constexpr auto
multiplication_OverflowUnsafe_SpecificBitCount(SWAR<NB, T> multiplicand,
  ...
  return associativeOperatorIterated_regressive(multiplicand, S{0}, shifted,
                                                S{S::MostSignificantBit},
                                                operation, ActualBits, halver);
}

/// \note Not removed yet because it is an example of "progressive" associative
/// exponentiation
template <int ActualBits, int NB, typename T>
...
template <int ActualBits, int NB, typename T>
constexpr auto
exponentiation_OverflowUnsafe_SpecificBitCount(SWAR<NB, T> x,
                                               SWAR<NB, T> exponent) {
  using S = SWAR<NB, T>;
  ...
  return associativeOperatorIterated_regressive(
      x, S{meta::BitmaskMaker<T, 1, NB>().value}, // neutral is lane wise..
      exponent, S{S::MostSignificantBit}, operation, ActualBits, halver);
}

template <int NB, typename T>
constexpr auto multiplication_OverflowUnsafe(SWAR<NB, T> multiplicand,
                                             SWAR<NB, T> multiplier) {
  return multiplication_OverflowUnsafe_SpecificBitCount<NB>(multiplicand,
                                                            multiplier);
}

template <int NB, typename T> struct SWAR_Pair {
  SWAR<NB, T> even, odd;
};
...
constexpr auto doublePrecision(SWAR<NB, T> input) {
  ...
  constexpr auto DM = S::evenLaneMask();
  return SWAR_Pair<NB * 2, T>{RV{(input & DM).value()},
...
constexpr auto halvePrecision(SWAR<NB, T> even, SWAR<NB, T> odd) {
  ...
  using RV = SWAR<NB / 2, T>;
  constexpr auto HalvingMask = RV::evenLaneMask();
  auto evenHalf = RV{even.value()} & HalvingMask,
