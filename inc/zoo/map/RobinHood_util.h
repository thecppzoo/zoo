...
struct RH {
    ...
    // Find slot can mean 'psl too short/no slot', 'found and in slot', 'not found but richer slot', 'not found but empty slot'
    MajorMinorInserted findSlot(Key k) {
      ...
      while(true) {
          ...
          while(matches) {
              ...
              if (m_.keyCheck(major + minor, k)) { return {major,minor, true};}
              matches = SM{matches.clearLSB()};
          }
          // minor points at slot that the key currently fits in.
          if (finished) return {major,minor, false};  // Finished no match, need slot, not zero.
      }
    }

    /// true if inserted, false if unable.
    /// int is position inserted to.
    template <typename KeyCheck> 
    std::pair<int, bool> insert(Key k, KeyCheck kc) {
      const auto twoNum = twoNumbers(k);
      const auto pos = twoNum.first;
      const auto hash = twoNum.second;
      const auto major = m_.posToSlot(pos);
      const auto haystack = m_.slotAt(pos);
      auto slotFind = findSlot(k);
      if (slotFind.inserted) {
          return {slotFind.major+slotFind.minor, false};  // already in
      }
