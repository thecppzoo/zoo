...
#define F false
#define T true
static_assert(BooleanSWAR{Literals<4, u16>, {F, F, F, F}}.value() ==
              0b0000'0000'0000'0000);
static_assert(BooleanSWAR{Literals<4, u16>, {T, F, F, F}}.value() ==
              0b1000'0000'0000'0000);
static_assert(BooleanSWAR{Literals<4, u16>, {F, T, F, F}}.value() ==
              0b0000'1000'0000'0000);
static_assert(BooleanSWAR{Literals<4, u16>, {F, F, T, F}}.value() ==
              0b0000'0000'1000'0000);
static_assert(BooleanSWAR{Literals<4, u16>, {F, F, F, T}}.value() ==
...
TEST_CASE("Compress/Expand", "[swar]") {
  ...
  SECTION("Regression 1") {
    u64 input =
            0b1010'1001'0110'0001'1001'0000'0010'1010'0100'0111'1110'1001'1111'0001'1110'1011,
        mask =
            0b0110'0000'0001'0101'0101'1111'0101'1100'0110'1111'0100'0111'0001'1000'0101'0010,
        expected =
            0b0001'0000'0000'0001'0001'0000'0000'0010'0010'0111'0001'0001'0001'0000'0010'0001;
    using S = S4_64;
...
#define GE_MSB_ON_TEST(left, right, result)                                    \
  static_assert(                                                               \
      result ==                                                                \
      greaterEqual<4, u32>(SWAR<4, u32>(left), SWAR<4, u32>(right)).value());

GE_MSB_ON_TEST(0x1000'0010, 0x0111'1101, 0x8000'0080)
GE_MSB_ON_TEST(0x4333'3343, 0x4444'4444, 0x8000'0080)
GE_MSB_ON_TEST(0x0550'0110, 0x0110'0550, 0x8888'8008)
GE_MSB_ON_TEST(0x4771'1414, 0x4641'1774, 0x8888'8008)
GE_MSB_ON_TEST(0x0123'4567, 0x0000'0000, 0x8888'8888)
GE_MSB_ON_TEST(0x0123'4567, 0x7777'7777, 0x0000'0008)
GE_MSB_ON_TEST(0x0000'0000, 0x0123'4567, 0x8000'0000)
GE_MSB_ON_TEST(0x7777'7777, 0x0123'4567, 0x8888'8888)

TEST_CASE("greaterEqualMSBOn", "[swar][unsigned-swar]") {
  SECTION("single") {
...
TEST_CASE("fullAddition", "[swar][signed-swar][unsigned-swar]") {
  SECTION("fullAddition overflow") {
    const auto sum =
        fullAddition(SWAR<4, u32>(0x0000'1000), SWAR<4, u32>(0x0000'7000));
  ...
  SECTION("no carry or overflow for safe values") {
    const auto sum =
        fullAddition(SWAR<4, u32>(0x0000'8000), SWAR<4, u32>(0x0000'7000));
    CHECK(SWAR<4, u32>(0x0000'0000).value() == sum.carry.value());
  ...
  SECTION("fullAddition signed overflow") {
    const auto sum =
        fullAddition(SWAR<4, u32>(0x0000'5000), SWAR<4, u32>(0x0000'5000));
    CHECK(SWAR<4, u32>(0x0000'0000).value() == sum.carry.value());
    CHECK(SWAR<4, u32>(0x0000'8000).value() == sum.overflow.value());
    CHECK(SWAR<4, u32>(0x0000'A000).value() == sum.result.value());
  }
  SECTION("0x0111 (7) + 0x0111 (7) is 0x1110 (0x1110->0x1101->0x0010) (0xe "
          "unsigned, 0x2 signed) (signed and unsigned check)") {
    const auto sum =
        fullAddition(SWAR<4, u32>(0x0000'7000), SWAR<4, u32>(0x0000'7000));
    CHECK(SWAR<4, u32>(0x0000'0000).value() == sum.carry.value());
    CHECK(SWAR<4, u32>(0x0000'8000).value() == sum.overflow.value());
  ...
  SECTION("both carry and overflow") {
    const auto sum =
        fullAddition(SWAR<4, u32>(0x0000'a000), SWAR<4, u32>(0x0000'a000));
    CHECK(SWAR<4, u32>(0x0000'8000).value() == sum.carry.value());
    CHECK(SWAR<4, u32>(0x0000'8000).value() == sum.overflow.value());
  }
}

TEST_CASE("BooleanSWAR MSBtoLaneMask", "[swar]") {
...
constexpr auto fullAddSumTest =
    fullAddition(S4_32(0x0111'1101), S4_32(0x1000'0010));
...
static_assert(S4_16(0x0000).value() ==
              saturatingUnsignedAddition(S4_16(0x0000), S4_16(0x0000)).value());
static_assert(S4_16(0x0200).value() ==
              saturatingUnsignedAddition(S4_16(0x0100), S4_16(0x0100)).value());
static_assert(S4_16(0x0400).value() ==
              saturatingUnsignedAddition(S4_16(0x0300), S4_16(0x0100)).value());
static_assert(S4_16(0x0A00).value() ==
              saturatingUnsignedAddition(S4_16(0x0300), S4_16(0x0700)).value());
static_assert(S4_16(0x0F00).value() ==
              saturatingUnsignedAddition(S4_16(0x0800), S4_16(0x0700)).value());
static_assert(S4_16(0x0F00).value() ==
              saturatingUnsignedAddition(S4_16(0x0800), S4_16(0x0800)).value());

TEST_CASE("saturatingUnsignedAddition", "[swar][saturation]") {
  CHECK(SWAR<4, u16>(0x0200).value() ==
        saturatingUnsignedAddition(SWAR<4, u16>(0x0100), SWAR<4, u16>(0x0100))
            .value());
  CHECK(SWAR<4, u16>(0x0400).value() ==
        saturatingUnsignedAddition(SWAR<4, u16>(0x0100), SWAR<4, u16>(0x0300))
            .value());
  CHECK(SWAR<4, u16>(0x0B00).value() ==
        saturatingUnsignedAddition(SWAR<4, u16>(0x0800), SWAR<4, u16>(0x0300))
            .value());
  CHECK(SWAR<4, u16>(0x0F00).value() ==
        saturatingUnsignedAddition(SWAR<4, u16>(0x0800), SWAR<4, u16>(0x0700))
            .value());
  CHECK(SWAR<4, u16>(0x0F00).value() ==
        saturatingUnsignedAddition(SWAR<4, u16>(0x0800), SWAR<4, u16>(0x0800))
            .value());
  CHECK(S4_32(0x0F0C'F000).value() ==
        saturatingUnsignedAddition(S4_32(0x0804'F000), S4_32(0x0808'F000))
            .value());
}
