...
In C++ 17 there are new library components `any`, `variant` and `optional` that have existed in the boost libraries for a while, `std::expected` is undergoing standarization, and `std::function` is an old friend.  They all have related functionality, they are types that lets you handle values of arbitrary types with the same interface.  Since the techniques to implement a much more powerful `any` than currently in the standard can be used to implement the other mentioned components, this document explains their functionality from the perspective of `any` and will serve as a basis to discuss them.

`any` allows holding a value of *any* type, hence the name.  It also allows holding no value.  It is conceptually similar to the concept of value or object in JavaScript, and I will try eventually implement it.  `any` allows the applicaton of type-less programming techniques.

There are use cases in which strong typing makes things harder.  For example, you may want to implement an event subscription/publishing mechanism that allows consumers of events of arbitrary types to subscribe to the same publisher.  You may build a processor of a non-trivial language to specify things like configurations; when the processor attempts to parse a particular configuration in that language, it does not know exactly what is the kind of objects that it will be generating.  You may want to implement a compiler for C++ in which you don't know what is the meaning of a new symbol until later (for example, an inline method that is used ahead of the declaration), so, you may want to have a "placeholder" to update after more information is known about the type of the object.
...
This is a popular choice, it is easy to understand, and requires less effort than `void *`.  It has plenty of applications, including my third implementation of `any`.  This choice may have maximal performance.  One inherent deficiency of this approach is that the whole set of types the values can have need to be known at compilation time (since they have to belong to the `union`, or C++ 17 `variant`), but this is not always feasible.  For example, I wrote a system in which trading strategies proprietary to the portfolio managers defined objects that my framework had to manage, without knowing what the type of those objects where.  There are more abstract and powerful choices than `union` or `variant`:
...
Another popular type switching mechanism, applicable to C and C++, is for "structs" to have an explicit type-switch member, which typically will be handled with a `switch` itself: `switch(object.typeId) { case TYPE1: ... }`.  One advantage that sometimes justifies all the extra programming effort for this kind of type switching is that the type switch can be as small as one bit.  Also, some commonality between the types can be achieved in cascading if-then-elses on the type id field.  As previously mentioned, the whole set of types needs to be programmed.  There are realistic use cases in which this is practical as well as cases in which it is not practical.

Curiously, both GCC libstdc++ and Clang's libc++ implement the type erasure needed in `any` by instantiating a template function which "knows" what is the type of the held object and internally has a switch with the many tasks required to manage the held object.  Perhaps the implementers did not use inheritance-virtual-method because of how cumbersome it is to have fine grain control of type switching when using it; however, by not using "polymorphic objects" directly, but working with them as raw bytes I have found fully portable (standard-compliant) ways to have very fine grained control of type switching, leading to the code of my implementation looking relatively straightforward.
